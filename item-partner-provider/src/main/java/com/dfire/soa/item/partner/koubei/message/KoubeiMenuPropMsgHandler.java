 ...
 */
@Component
@MessageTag(tag = {CommonConstant.Notify.MENU_PROP_INSERT_TAG, CommonConstant.Notify.MENU_PROP_UPDATE_TAG})
public class KoubeiMenuPropMsgHandler implements ConsumerCallBack {
    @Resource
    ...
    public boolean process(AsyncMsg msg) {
        ...
        try{
            ...
            MenuProp menuPropafter = JSON.parseObject(dataMap.get("data_after"), MenuProp.class);

            //口碑-菜品tag处理
            switch (tag){
                case CommonConstant.Notify.MENU_PROP_INSERT_TAG://菜品附属信息-新增
                    return koubeiDishHandler.pushkoubeiDishAddAndUpdate(merchantId, shopId, menuPropafter.getId(),menuPropafter.getEntityId(),0);
                case CommonConstant.Notify.MENU_PROP_UPDATE_TAG://菜品附属信息-修改
                    return koubeiDishHandler.pushkoubeiDishAddAndUpdate(merchantId, shopId, menuPropafter.getId(),menuPropafter.getEntityId(),0);

            }
        }catch (BizException e) {
            bizLog.warn("[kb_databack]KoubeiMenuPropMsgHandler.process(msg) BizException. tag: " + JSON.toJSONString(tag) + ", dataMap:" + JSON.toJSONString(dataMap), e);
            return true;
        }catch (Exception e) {
            bizLog.error("[kb_databack]KoubeiMenuPropMsgHandler.process(msg) Exception. tag: " + JSON.toJSONString(tag) + ", dataMap:" + JSON.toJSONString(dataMap), e);
            return false;
        }
        return true;
    }
